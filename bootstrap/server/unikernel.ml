    ...
    let port = Key_gen.port () in
      S.listen_tcpv4 s ~port (
        fun flow ->
          ...
          let greeting = Greeting.new_greeting "NULL" |> Greeting.to_bytes in
            ...
            | Ok () -> (S.TCPV4.read flow >>= function
              ...
              | Ok (`Data b) -> Logs.debug (fun f -> f "read: %d bytes:\n%s" (Cstruct.len b)  (buffer_to_string (Cstruct.to_bytes b))); S.TCPV4.close flow)
